...
// Base interface for draggable/togglable items
interface BaseItem {
  id: string;
  isActive: boolean;
  order: number;
...
export function AdminLandingPage() {
  ...
  const [howItWorksData, setHowItWorksData] = useState<HowItWorksContent | null>(null);
  const [socialProofData, setSocialProofData] = useState<SocialProofContent | null>(null);
    ...
    try {
      ...
      sectionsArray.forEach((section) => {
        if (section.key === 'hero') {
          ...
          setFeaturesData({
            ...content,
            features: ensureItemFields<Feature>(content.features || []),
          });
        } else if (section.key === 'cta') {
          setCtaData(section.content as CTAContent);
        } else if (section.key === 'how-it-works') {
          const content = section.content as any;
          setHowItWorksData({
            ...content,
            steps: ensureItemFields<HowItWorksStep>(content.steps || []),
            stats: ensureItemFields<HowItWorksStat>(content.stats || []),
          });
        } else if (section.key === 'social-proof') {
          const content = section.content as any;
          setSocialProofData({
            ...content,
            stats: ensureItemFields<SocialProofStat>(content.stats || []),
            logos: ensureItemFields<SocialProofLogo>(content.logos || []),
            testimonials: ensureItemFields<SocialProofTestimonial>(content.testimonials || []),
          });
        }
      });
    } catch (error: any) {
  ...
  const handleSaveSection = async (sectionKey: string) => {
    try {
      ...
      else if (sectionKey === 'how-it-works') content = howItWorksData;
      else if (sectionKey === 'social-proof') content = socialProofData;

      // Get the updated section from the API response
      const updatedSection = await adminLandingService.update(section.id, { content });

      // Update only the changed section in state (no refetch needed)
      setSections(prevSections =>
        prevSections.map(s => s.id === updatedSection.id ? updatedSection : s)
      );

      // Update the specific data state to ensure consistency
      if (sectionKey === 'hero') setHeroData(updatedSection.content as HeroContent);
      else if (sectionKey === 'features') setFeaturesData(updatedSection.content as FeaturesContent);
      else if (sectionKey === 'cta') setCtaData(updatedSection.content as CTAContent);
      else if (sectionKey === 'how-it-works') setHowItWorksData(updatedSection.content as HowItWorksContent);
      else if (sectionKey === 'social-proof') setSocialProofData(updatedSection.content as SocialProofContent);
  ...
  const handleToggleActive = async (section: LandingSection) => {
    try {
      const updatedSection = await adminLandingService.toggleActive(section.id, !section.isActive);

      // Update only the toggled section in state
      setSections(prevSections =>
        prevSections.map(s => s.id === updatedSection.id ? updatedSection : s)
      );
      ...
      setMessage({
        type: 'success',
        text: `${section.key} section ${updatedSection.isActive ? 'activated' : 'deactivated'}`
      });
    } catch (error: any) {
          ...
          <Tabs defaultValue={sections[0]?.key || 'hero'} className="w-full">
            ...
            <TabsContent value="features" className="mt-6">
              {featuresData && (
                <FeaturesEditor
                  data={featuresData}
                  sectionIsActive={sections.find(s => s.key === 'features')?.isActive || false}
                  onToggleSection={() => handleToggleActive(sections.find(s => s.key === 'features')!)}
            ...
            <TabsContent value="how-it-works" className="mt-6">
              {howItWorksData && (
                <HowItWorksEditor
                  data={howItWorksData}
                  sectionIsActive={sections.find(s => s.key === 'how-it-works')?.isActive || false}
                  onToggleSection={() => handleToggleActive(sections.find(s => s.key === 'how-it-works')!)}
                  onSave={() => handleSaveSection('how-it-works')}
                  onChange={setHowItWorksData}
                  saving={saving}
                />
              )}
            </TabsContent>
            ...
            <TabsContent value="social-proof" className="mt-6">
              {socialProofData && (
                <SocialProofEditor
                  ...
                  sectionIsActive={sections.find(s => s.key === 'social-proof')?.isActive || false}
                  onToggleSection={() => handleToggleActive(sections.find(s => s.key === 'social-proof')!)}
                  onSave={() => handleSaveSection('social-proof')}
                  onChange={setSocialProofData}
                  saving={saving}
                />
              )}
            </TabsContent>
