...
import { useCustomer, useGoogleConnectionStatus } from '@/hooks/useCustomers';
...
export default function CustomerDetail() {
  ...
  const [editingTracking, setEditingTracking] = React.useState<Tracking | undefined>();

  const { data: customer, isLoading: isLoadingCustomer, error, refetch } = useCustomer(customerId!);
  const { data: connectionStatus, isLoading: isLoadingConnectionStatus } = useGoogleConnectionStatus(customerId!);
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
                ...
                {isLoadingConnectionStatus ? (
                  <>
                    ...
                    <div className="flex items-center space-x-1 px-2 py-1 bg-blue-50 border border-blue-200 rounded">
                      ...
                      <span className="text-xs font-medium text-blue-700">GTM</span>
                    </div>
                    {/* GA4 Status - Syncing */}
                    <div className="flex items-center space-x-1 px-2 py-1 bg-blue-50 border border-blue-200 rounded">
                      <Loader2 className="h-3 w-3 text-blue-600 animate-spin" />
                      <span className="text-xs font-medium text-blue-700">GA4</span>
                    </div>
                    {/* Ads Status - Syncing */}
                    <div className="flex items-center space-x-1 px-2 py-1 bg-blue-50 border border-blue-200 rounded">
                      <Loader2 className="h-3 w-3 text-blue-600 animate-spin" />
                      <span className="text-xs font-medium text-blue-700">Ads</span>
                    </div>
                  </>
                  ...
                  <div className="flex items-center space-x-1 px-2 py-1 bg-yellow-50 border border-yellow-200 rounded">
                    ...
                    <span className="text-xs font-medium text-yellow-700">Not Connected</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button
            ...
            disabled={syncAllTrackings.isPending}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg border p-4">
          ...
          {isLoadingCustomer ? (
            ...
            <p className="text-2xl font-bold">{customer?.totalCampaigns ?? 0}</p>
          )}
        </div>
        <div className="bg-white rounded-lg border p-4">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="trackings" className="flex items-center space-x-2">
            <List className="h-4 w-4" />
            <span>Current Trackings</span>
          </TabsTrigger>
          <TabsTrigger value="create-track" className="flex items-center space-x-2">
